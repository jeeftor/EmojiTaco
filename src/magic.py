...
def main(wf: Workflow3) -> None:
    ...
    wf.add_item(
        "workflow:openlog",
        "Open the Workflow’s log file in the default app.",
    ...
    wf.add_item(
        "workflow:version",
        "Display the installed version of the workflow (if one is set).",
        arg="workflow:version",
        autocomplete="workflow:version",
        icon=ICON_INFO,
    )
    wf.add_item(
        "workflow:delcache",
        "Delete the Workflow’s cache.",
        arg="workflow:delcache",
        autocomplete="workflow:delcache",
        icon=ICON_INFO,
    )
    wf.add_item(
        "workflow:deldata",
        "Delete the Workflow’s saved data.",
        arg="workflow:deldata",
        autocomplete="workflow:deldata",
        icon=ICON_INFO,
    )
    wf.add_item(
        "workflow:delsettings",
        "Delete the Workflow’s settings file (which contains the data stored using Workflow.settings).",
        arg="workflow:delsettings",
        autocomplete="workflow:delsettings",
        icon=ICON_INFO,
    )
    wf.add_item(
        "workflow:foldingdefault",
        "Reset diacritic folding to workflow default",
        arg="workflow:foldingdefault",
        autocomplete="workflow:foldingdefault",
        icon=ICON_INFO,
    )
    wf.add_item(
        "workflow:foldingoff",
        "Never fold diacritics in search keys",
        arg="workflow:foldingoff",
        autocomplete="workflow:foldingoff",
        icon=ICON_INFO,
    )
    wf.add_item(
        "workflow:foldingon",
        "Force diacritic folding in search keys (e.g. convert ü to ue)",
        arg="workflow:foldingon",
        autocomplete="workflow:foldingon",
        icon=ICON_INFO,
    )
    wf.add_item(
        "workflow:opencache",
        "Open the Workflow’s cache directory.",
        arg="workflow:opencache",
        autocomplete="workflow:opencache",
        icon=ICON_INFO,
    )
    wf.add_item(
        "workflow:opendata",
        "Open the Workflow’s data directory.",
        arg="workflow:opendata",
        autocomplete="workflow:opendata",
        icon=ICON_INFO,
    )
    wf.add_item(
        "workflow:openterm",
        "Open a Terminal window in the Workflow’s root directory.",
        arg="workflow:openterm",
        autocomplete="workflow:openterm",
        icon=ICON_INFO,
    )
    wf.add_item(
        "workflow:openworkflow",
        "Open the Workflow’s root directory (where info.plist is).",
        arg="workflow:openworkflow",
        autocomplete="workflow:openworkflow",
        icon=ICON_INFO,
    )
    wf.add_item(
        "workflow:reset",
        "Delete the Workflow’s settings, cache and saved data.",
        arg="workflow:reset",
    ...
    wf.add_item(
        ...
        "Turn off automatic checks for updates.",
        arg="workflow:noautoupdate",
        autocomplete="workflow:noautoupdate",
        icon=ICON_INFO,
    )
    wf.add_item(
        "workflow:autoupdate",
...
if __name__ == "__main__":
    wf = Workflow3(
        libraries=["./lib"],
        help_url=HELP_URL,
        update_settings={"github_slug": "jeeftor/alfredToday", "frequency": 7},
    )
    log = wf.logger
    wf.run(main)
